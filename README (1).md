...
A minimal **courier**, not a full RAT. It fingerprints the host, registers with a C2, waits for an operator "go" verdict, downloads a three-file second stage, launches it, and self-deletes. No persistence, no privilege escalation, no obfuscation. Full teardown in [`docs/TECHNICAL_ANALYSIS.md`](docs/TECHNICAL_ANALYSIS.md).
...
| `docs/TECHNICAL_ANALYSIS.md` | **Static RE writeup** — kill chain, function map, C2 protocol, tooling cross-check |
